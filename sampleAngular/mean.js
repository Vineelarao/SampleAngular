...
//    db = mongojs("js_june", ['rao','jobPosts']);

var mongoose = require('mongoose');

var expressSession = require('express-session');
app.use(expressSession({secret:'Vineela', saveUninitialized: false, resave: false}));
...
app.post('/postData', function(req, res) {
    ...
    mongo_conn.collection('rao').insert(doc, function(err, response) {
        if(response) {
...
app.get('/getMongoData', function(req, res) {
    var temp = [];
    db.collection('jobPosts').find({}).toArray(function(err, doc){
        temp = doc;
        res.json(temp);
    });  
//    console.log(temp);
